...
generate_timestamp_serial() {
    ...
    timestamp=$(date +%s)
    local random
    random=$(head -c 4 /dev/urandom | od -An -tu4 | tr -d ' ')
...
main() {
    ...
    local serial1
    serial1=$(generate_hex_serial_with_prefix 4)
    if test_ca_serial "hex-0x-4b" "${serial1}" "Hex with 0x prefix, 4 bytes" "test-ca-hex-0x-4b"; then
        ...
        local serial2
        serial2=$(generate_hex_serial_with_prefix 4)
